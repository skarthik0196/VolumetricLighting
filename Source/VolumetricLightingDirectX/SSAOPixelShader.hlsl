...
float4 main(VS_OUTPUT Input) : SV_TARGET
{
	...
	//	SamplePosition = Position + SamplePosition * RadiusAdjust;
	...
	//	float RangeCheck = abs(Position.z - SampleDepth) < Radius ? 1.0 : 0.0;//smoothstep(0.0f, 1.0f, Radius / abs(Position.z - SampleDepth));

	//	//Occlusion += ((SampleDepth + 0.025f) >= (SamplePosition.z) ? 1.0f : 0.0f) * RangeCheck;
